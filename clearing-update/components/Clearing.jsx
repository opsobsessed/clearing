...
import {
  ...
  LifeBuoy, Phone, MessageCircle, ChevronDown, ChevronUp, AlertTriangle, Scale, Camera
} from "lucide-react";
...
function IncidentForm({ onSave, onCancel, references = [], workplaceContact = "" }) {
  const [f, setF] = useState({ date: localDay(), channel: "Call", target: "Me", refName: "", agentName: "", recorded: false, notes: "" });
  return (
...
function ComplaintForm({ onSave, onCancel }) {
  const [f, setF] = useState({ date: localDay(), filedWith: COMPLAINT_TARGETS[0], refNumber: "", status: "Filed", notes: "" });
  return (
    <div style={{ display: "grid", gap: 8, marginTop: 8 }}>
      <input className="in" type="date" value={f.date} onChange={e => setF({ ...f, date: e.target.value })} />
...
function SettlementForm({ onSave, onCancel }) {
  const [f, setF] = useState({ date: localDay(), offeredAmount: "", accepted: false, notes: "" });
  return (
    <div style={{ display: "grid", gap: 8, marginTop: 8 }}>
      <div className="row" style={{ gap: 8 }}>
